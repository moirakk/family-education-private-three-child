...
export function FamilyEventPlanner({ childProfiles, existingEvents = [], onEventsChange, openFormRequest = 0 }: {
  ...
  return <Card id="event-planner" className="border-border bg-card shadow-none"><CardHeader><div className="flex items-center justify-between gap-3"><CardTitle className="flex items-center gap-2"><CalendarPlus className="h-4 w-4" />日程</CardTitle><Button onClick={() => setShowForm(true)}><CalendarPlus className="mr-2 h-4 w-4" />新增日程</Button></div></CardHeader><CardContent className="space-y-4">
    ...
    {showForm ? <form onSubmit={save} className="rounded-2xl border border-border p-4"><div className="mb-4 flex items-center justify-between"><p className="font-medium">{editingId ? "编辑日程" : "新增日程"}</p><button type="button" aria-label="关闭表单" className="-m-2 flex h-11 w-11 items-center justify-center rounded-full text-muted-foreground hover:bg-muted hover:text-foreground" onClick={reset}><X className="h-4 w-4" /></button></div><div className="grid gap-4">
      <div><Label>孩子 *</Label><div className="mt-2 grid grid-cols-3 gap-2">{childProfiles.map((child) => <button key={child.id} type="button" onClick={() => toggleChild(child.id)} className={cn("min-h-11 rounded-xl border p-2.5 text-sm", form.childIds.includes(child.id) ? "border-foreground bg-foreground text-background" : "border-border")}>{child.firstName}</button>)}</div></div>
      <div><Label>类型 *</Label><div className="mt-2 grid grid-cols-3 gap-2">{categories.map((item) => <button key={item.value} type="button" onClick={() => setForm((v) => ({ ...v, category: item.value }))} className={cn("min-h-11 rounded-xl border p-2.5 text-sm", form.category === item.value ? "border-foreground bg-foreground text-background" : "border-border")}>{item.label}</button>)}</div></div>
      ...
      {!form.allDay ? <div><Label>时长 *</Label><div className="mt-2 flex gap-2 overflow-x-auto pb-1">{durations.map((item) => <button key={item.value} type="button" onClick={() => setForm((v) => ({ ...v, duration: item.value }))} className={cn("min-h-11 shrink-0 rounded-full border px-4 py-1.5 text-xs sm:min-h-9 sm:px-3", form.duration === item.value ? "border-foreground bg-foreground text-background" : "border-border")}>{item.label}</button>)}</div>{form.duration === "custom" ? <Input className="mt-2" type="number" min="1" inputMode="numeric" placeholder="分钟" value={form.customDuration} onChange={(e) => setForm((v) => ({ ...v, customDuration: e.target.value }))} /> : null}</div> : null}
