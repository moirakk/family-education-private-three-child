...
export function FamilyEventPlanner({ childProfiles, existingEvents = [], onEventsChange, openFormRequest = 0 }: {
  ...
  return <Card id="event-planner" className="border-border bg-card shadow-none"><CardHeader><div className="flex items-center justify-between gap-3"><CardTitle className="flex items-center gap-2"><CalendarPlus className="h-4 w-4" />日程</CardTitle><Button onClick={() => setShowForm(true)}><CalendarPlus className="mr-2 h-4 w-4" />新增日程</Button></div></CardHeader><CardContent className="space-y-4">
    <div className="flex gap-2 overflow-x-auto">{[{ id: "all", firstName: "全部" }, ...childProfiles].map((child) => <button key={child.id} type="button" onClick={() => setChildFilter(child.id)} className={cn("shrink-0 rounded-full border px-3 py-1.5 text-xs", childFilter === child.id ? "border-foreground bg-foreground text-background" : "border-border")}>{child.firstName}</button>)}</div>
    ...
    {history.length ? <details className="rounded-2xl border border-border p-3"><summary className="cursor-pointer text-sm font-medium">历史日程（{history.length}）</summary><div className="mt-3"><EventList items={history} childProfiles={childProfiles} edit={edit} confirmDeleteId={confirmDeleteId} setConfirmDeleteId={setConfirmDeleteId} remove={remove} /></div></details> : null}
    {status ? <p className="text-xs text-muted-foreground">{status}</p> : null}
    {showForm ? <form onSubmit={save} className="rounded-2xl border border-border p-4"><div className="mb-4 flex items-center justify-between"><p className="font-medium">{editingId ? "编辑日程" : "新增日程"}</p><button type="button" onClick={reset}><X className="h-4 w-4" /></button></div><div className="grid gap-4">
      <div><Label>孩子 *</Label><div className="mt-2 grid grid-cols-3 gap-2">{childProfiles.map((child) => <button key={child.id} type="button" onClick={() => toggleChild(child.id)} className={cn("rounded-xl border p-2.5 text-sm", form.childIds.includes(child.id) ? "border-foreground bg-foreground text-background" : "border-border")}>{child.firstName}</button>)}</div></div>
      <div><Label>类型 *</Label><div className="mt-2 grid grid-cols-3 gap-2">{categories.map((item) => <button key={item.value} type="button" onClick={() => setForm((v) => ({ ...v, category: item.value }))} className={cn("rounded-xl border p-2.5 text-sm", form.category === item.value ? "border-foreground bg-foreground text-background" : "border-border")}>{item.label}</button>)}</div></div>
      <div><Label htmlFor="event-title">事项名称 *</Label><Input id="event-title" value={form.title} onChange={(e) => setForm((v) => ({ ...v, title: e.target.value }))} /></div>
      ...
      <div className={cn("grid gap-3", !form.allDay && "grid-cols-2")}><div><Label htmlFor="event-date">日期 *</Label><Input id="event-date" type="date" value={form.date} onChange={(e) => setForm((v) => ({ ...v, date: e.target.value }))} /></div>{!form.allDay ? <div><Label htmlFor="event-time">开始时间 *</Label><Input id="event-time" type="time" value={form.startTime} onChange={(e) => setForm((v) => ({ ...v, startTime: e.target.value }))} /></div> : null}</div>
      {!form.allDay ? <div><Label>时长 *</Label><div className="mt-2 flex gap-2 overflow-x-auto">{durations.map((item) => <button key={item.value} type="button" onClick={() => setForm((v) => ({ ...v, duration: item.value }))} className={cn("shrink-0 rounded-full border px-3 py-1.5 text-xs", form.duration === item.value ? "border-foreground bg-foreground text-background" : "border-border")}>{item.label}</button>)}</div>{form.duration === "custom" ? <Input className="mt-2" type="number" min="1" inputMode="numeric" placeholder="分钟" value={form.customDuration} onChange={(e) => setForm((v) => ({ ...v, customDuration: e.target.value }))} /> : null}</div> : null}
      <button type="button" className="w-fit text-sm font-medium text-primary" onClick={() => setAdvanced((value) => !value)}>{advanced ? "收起更多设置" : "更多设置：地点、备注、重复"}</button>
      {advanced ? <div className="grid gap-4 rounded-2xl bg-muted/40 p-3"><div><Label htmlFor="event-location">地点</Label><Input id="event-location" value={form.location} onChange={(e) => setForm((v) => ({ ...v, location: e.target.value }))} /></div><div><Label htmlFor="event-notes">备注</Label><Textarea id="event-notes" rows={2} value={form.notes} onChange={(e) => setForm((v) => ({ ...v, notes: e.target.value }))} /></div>
        <div><Label>重复</Label><div className="mt-2 flex gap-2 overflow-x-auto">{repeats.map((item) => <button key={item.value} type="button" onClick={() => setForm((v) => ({ ...v, repeat: item.value }))} className={cn("shrink-0 rounded-full border px-3 py-1.5 text-xs", form.repeat === item.value ? "border-foreground bg-foreground text-background" : "border-border")}>{item.label}</button>)}</div></div>
        {form.repeat !== "none" ? <><div><Label htmlFor="repeat-interval">间隔</Label><Input id="repeat-interval" type="number" min="1" value={form.repeatInterval} onChange={(e) => setForm((v) => ({ ...v, repeatInterval: e.target.value }))} /></div><div><Label>结束重复</Label><select className="mt-2 h-10 w-full rounded-xl border border-border bg-card px-3 text-sm" value={form.repeatEnd} onChange={(e) => setForm((v) => ({ ...v, repeatEnd: e.target.value as FormState["repeatEnd"] }))}><option value="never">永不</option><option value="date">指定日期</option><option value="count">指定次数</option></select></div>{form.repeatEnd === "date" ? <Input type="date" value={form.repeatEndDate} onChange={(e) => setForm((v) => ({ ...v, repeatEndDate: e.target.value }))} /> : null}{form.repeatEnd === "count" ? <Input type="number" min="1" value={form.repeatCount} onChange={(e) => setForm((v) => ({ ...v, repeatCount: e.target.value }))} /> : null}</> : null}
      </div> : null}
      <div className="rounded-xl bg-muted/50 p-3 text-sm"><p className="font-medium">保存预览</p><p className="mt-1 text-muted-foreground">{form.date}{form.allDay ? " · 全天" : ` · ${form.startTime} · ${durationMinutes(form)} 分钟`}{form.repeat !== "none" ? ` · ${repeats.find((item) => item.value === form.repeat)?.label}` : ""}</p></div>
...
function EventList({ title, items, childProfiles, edit, confirmDeleteId, setConfirmDeleteId, remove }: { title?: string; items: CalendarEvent[]; childProfiles: Child[]; edit: (event: CalendarEvent) => void; confirmDeleteId: string | null; setConfirmDeleteId: (id: string | null) => void; remove: (id: string) => void }) {
  ...
  return <div className="space-y-2">{title ? <p className="text-sm font-medium">{title}</p> : null}{items.map((event) => <div key={event.id} className="rounded-2xl border border-border p-3"><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><Badge variant="outline">{categories.find((item) => item.value === event.category)?.label ?? "历史"}</Badge><p className="font-medium">{event.title}</p></div><p className="mt-1 text-xs text-muted-foreground">{formatEvent(event)}{event.location ? ` · ${event.location}` : ""}</p><p className="mt-1 text-xs text-muted-foreground">{event.childIds.map((id) => childById.get(id)).filter(Boolean).join("、")}{event.recurrenceRule ? " · 重复" : ""}</p></div>{confirmDeleteId === event.id ? <div className="flex flex-col gap-1"><Button size="sm" variant="destructive" onClick={() => remove(event.id)}>确认删除</Button><Button size="sm" variant="ghost" onClick={() => setConfirmDeleteId(null)}>取消</Button></div> : <div className="flex"><Button size="icon" variant="ghost" onClick={() => edit(event)}><Pencil className="h-4 w-4" /></Button><Button size="icon" variant="ghost" onClick={() => setConfirmDeleteId(event.id)}><Trash2 className="h-4 w-4" /></Button></div>}</div>{event.notes ? <p className="mt-2 text-xs text-muted-foreground">{event.notes}</p> : null}</div>)}{!items.length ? <p className="rounded-xl bg-muted/50 p-4 text-sm text-muted-foreground">暂无日程。</p> : null}</div>;
}
