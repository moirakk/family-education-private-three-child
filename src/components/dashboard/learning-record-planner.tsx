    ...
    <Card id="learning-records">
      <CardHeader className="gap-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-xl border border-border/60 bg-gradient-to-br from-muted/60 to-muted/30 p-3"><p className="text-xs font-medium text-muted-foreground">最近</p><p className="mt-1 text-lg font-semibold tabular-nums tracking-tight">{scored[0] ?? "-"}%</p></div>
          <div className="rounded-xl border border-border/60 bg-gradient-to-br from-muted/60 to-muted/30 p-3"><p className="text-xs font-medium text-muted-foreground">平均</p><p className="mt-1 text-lg font-semibold tabular-nums tracking-tight">{average ?? "-"}%</p></div>
          <div className="rounded-xl border border-border/60 bg-gradient-to-br from-muted/60 to-muted/30 p-3"><p className="text-xs font-medium text-muted-foreground">最高</p><p className="mt-1 text-lg font-semibold tabular-nums tracking-tight text-primary">{highest ?? "-"}%</p></div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          {visible.map((record) => <div key={record.id} className="rounded-2xl border border-border/70 bg-card p-3 shadow-sm shadow-black/[0.03] transition-shadow duration-200 hover:shadow-md hover:shadow-black/[0.06]">
            ...
            {record.notes ? <p className="mt-2 text-xs leading-5 text-muted-foreground">{record.notes}</p> : null}
          </div>)}
          {!visible.length ? <p className="rounded-xl border border-dashed border-amber-200/70 bg-gradient-to-br from-amber-50/60 to-rose-50/40 p-5 text-center text-sm text-muted-foreground dark:border-white/15 dark:from-white/[0.05] dark:to-white/[0.03]">还没有符合条件的成绩，换个筛选试试。</p> : null}
        </div>
