    ...
    <Card id="learning-records" className="border-border bg-card shadow-none">
      ...
      <CardContent className="space-y-4">
        <div className="flex gap-2 overflow-x-auto pb-1">
          {[{ id: "all", firstName: "全部" }, ...childProfiles].map((child) => <button key={child.id} type="button" onClick={() => setChildFilter(child.id)} className={cn("shrink-0 rounded-full border px-3 py-1.5 text-xs", childFilter === child.id ? "border-foreground bg-foreground text-background" : "border-border")}>{child.firstName}</button>)}
        </div>
        <div className="grid gap-2 sm:grid-cols-3">
          <select className="h-10 rounded-xl border border-border bg-card px-3 text-sm" value={subjectFilter} onChange={(e) => setSubjectFilter(e.target.value)}><option value="all">全部科目</option>{subjects.map((item) => <option key={item}>{item}</option>)}</select>
          <select className="h-10 rounded-xl border border-border bg-card px-3 text-sm" value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}><option value="all">全部考试</option>{examTypes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
        ...
        {showForm ? <form onSubmit={save} className="rounded-2xl border border-border p-4"><div className="mb-4 flex items-center justify-between"><p className="font-medium">{editingId ? "编辑成绩" : "录入成绩"}</p><button type="button" onClick={reset}><X className="h-4 w-4" /></button></div><div className="grid gap-4">
          ...
          <div><Label>科目 *</Label><div className="mt-2 grid grid-cols-4 gap-2">{subjects.map((subject) => <button key={subject} type="button" onClick={() => setForm((v) => ({ ...v, subject }))} className={cn("rounded-xl border p-2 text-sm", form.subject === subject ? "border-foreground bg-foreground text-background" : "border-border")}>{subject}</button>)}</div>{form.subject === "其他" ? <Input className="mt-2" placeholder="输入科目" value={form.customSubject} onChange={(e) => setForm((v) => ({ ...v, customSubject: e.target.value }))} /> : null}</div>
          ...
          <div><Label>考试类型</Label><div className="mt-2 flex gap-2 overflow-x-auto">{examTypes.map((item) => <button key={item.value} type="button" onClick={() => setForm((v) => ({ ...v, examType: item.value }))} className={cn("shrink-0 rounded-full border px-3 py-1.5 text-xs", form.examType === item.value ? "border-foreground bg-foreground text-background" : "border-border")}>{item.label}</button>)}</div></div>
