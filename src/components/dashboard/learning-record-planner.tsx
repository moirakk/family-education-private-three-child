    ...
    <Card id="learning-records">
      <CardHeader className="gap-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-xl border border-border/60 bg-muted/40 p-3"><p className="text-xs font-medium text-muted-foreground">最近</p><p className="mt-1 text-lg font-semibold tabular-nums tracking-tight">{scored[0] ?? "-"}%</p></div>
          <div className="rounded-xl border border-border/60 bg-muted/40 p-3"><p className="text-xs font-medium text-muted-foreground">平均</p><p className="mt-1 text-lg font-semibold tabular-nums tracking-tight">{average ?? "-"}%</p></div>
          <div className="rounded-xl border border-border/60 bg-muted/40 p-3"><p className="text-xs font-medium text-muted-foreground">最高</p><p className="mt-1 text-lg font-semibold tabular-nums tracking-tight text-primary">{highest ?? "-"}%</p></div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="flex gap-2 overflow-x-auto pb-1">
          {[{ id: "all", firstName: "全部" }, ...childProfiles].map((child) => <button key={child.id} type="button" aria-pressed={childFilter === child.id} onClick={() => setChildFilter(child.id)} className={cn("min-h-11 shrink-0 rounded-full border px-4 py-1.5 text-xs sm:min-h-9 sm:px-3", childFilter === child.id ? "border-primary bg-primary text-primary-foreground shadow-sm shadow-primary/25 transition-colors duration-200" : "border-border transition-colors duration-200 hover:border-muted-foreground/40 hover:bg-muted/50")}>{child.firstName}</button>)}
        </div>
        <div className="grid gap-2 sm:grid-cols-3">
          <select aria-label="按科目筛选" className="h-11 rounded-xl border border-border bg-card px-3 text-base sm:h-10 sm:text-sm" value={subjectFilter} onChange={(e) => setSubjectFilter(e.target.value)}><option value="all">全部科目</option>{subjects.map((item) => <option key={item}>{item}</option>)}</select>
          <select aria-label="按考试类型筛选" className="h-11 rounded-xl border border-border bg-card px-3 text-base sm:h-10 sm:text-sm" value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}><option value="all">全部考试</option>{examTypes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
        ...
        {scored.length >= 3 ? <div className="rounded-2xl border border-border/70 bg-muted/20 p-3"><p className="flex items-center gap-2 text-sm font-semibold tracking-tight"><TrendingUp className="h-4 w-4 text-primary" />成绩趋势</p><div className="mt-3 flex h-24 items-end gap-2">{scored.slice(0, 12).reverse().map((value, index) => <div key={`${value}-${index}`} className="min-w-2 flex-1 rounded-t-md bg-gradient-to-t from-primary/50 to-primary/90 transition-opacity duration-200 hover:opacity-80" style={{ height: `${Math.max(8, Math.min(100, value))}%` }} title={`${value}%`} />)}</div></div> : null}
        <div className="space-y-2">
          {visible.map((record) => <div key={record.id} className="rounded-2xl border border-border/70 bg-card p-3 shadow-sm shadow-black/[0.03] transition-shadow duration-200 hover:shadow-md hover:shadow-black/[0.06]">
            <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap gap-2"><Badge variant="outline">{childById.get(record.childId)}</Badge><Badge variant="secondary">{record.subject}</Badge><span className="text-xs tabular-nums text-muted-foreground">{record.date}</span></div><p className="mt-2 font-medium">{record.title}</p><p className="mt-1 text-sm tabular-nums text-muted-foreground">{record.score ?? "-"} / {record.maxScore ?? 100} · {percentage(record) ?? "-"}%</p></div>
        ...
        {showForm ? <form onSubmit={save} className="rounded-2xl border border-border/70 bg-muted/20 p-4 shadow-sm shadow-black/[0.03]"><div className="mb-4 flex items-center justify-between"><p className="font-semibold tracking-tight">{editingId ? "编辑成绩" : "录入成绩"}</p><button type="button" aria-label="关闭表单" className="-m-2 flex h-11 w-11 items-center justify-center rounded-full text-muted-foreground transition-colors duration-200 hover:bg-muted hover:text-foreground" onClick={reset}><X className="h-4 w-4" /></button></div><div className="grid gap-4">
          ...
          <div><Label>科目 *</Label><div className="mt-2 grid grid-cols-4 gap-2">{subjects.map((subject) => <button key={subject} type="button" aria-pressed={form.subject === subject} onClick={() => setForm((v) => ({ ...v, subject }))} className={cn("min-h-11 rounded-xl border p-2 text-sm", form.subject === subject ? "border-primary bg-primary text-primary-foreground shadow-sm shadow-primary/25 transition-colors duration-200" : "border-border transition-colors duration-200 hover:border-muted-foreground/40 hover:bg-muted/50")}>{subject}</button>)}</div>{form.subject === "其他" ? <Input className="mt-2" aria-label="自定义科目" placeholder="输入科目" value={form.customSubject} onChange={(e) => setForm((v) => ({ ...v, customSubject: e.target.value }))} /> : null}</div>
          <div className="grid gap-3 sm:grid-cols-2"><div><Label htmlFor="exam-title">考试名称 *</Label><Input id="exam-title" className="mt-2" value={form.title} onChange={(e) => setForm((v) => ({ ...v, title: e.target.value }))} /></div><div><Label htmlFor="exam-date">日期 *</Label><Input id="exam-date" className="mt-2" type="date" value={form.date} onChange={(e) => setForm((v) => ({ ...v, date: e.target.value }))} /></div></div>
          <div className="grid grid-cols-2 gap-3"><div><Label htmlFor="score">得分 *</Label><Input id="score" className="mt-2" inputMode="decimal" type="number" min="0" step="0.01" value={form.score} onChange={(e) => setForm((v) => ({ ...v, score: e.target.value }))} /></div><div><Label htmlFor="max-score">满分 *</Label><Input id="max-score" className="mt-2" inputMode="decimal" type="number" min="1" step="0.01" value={form.maxScore} onChange={(e) => setForm((v) => ({ ...v, maxScore: e.target.value }))} /></div></div>
          <div><Label>考试类型</Label><div className="mt-2 flex gap-2 overflow-x-auto pb-1">{examTypes.map((item) => <button key={item.value} type="button" aria-pressed={form.examType === item.value} onClick={() => setForm((v) => ({ ...v, examType: item.value }))} className={cn("min-h-11 shrink-0 rounded-full border px-4 py-1.5 text-xs sm:min-h-9 sm:px-3", form.examType === item.value ? "border-primary bg-primary text-primary-foreground shadow-sm shadow-primary/25 transition-colors duration-200" : "border-border transition-colors duration-200 hover:border-muted-foreground/40 hover:bg-muted/50")}>{item.label}</button>)}</div></div>
          <div><Label htmlFor="score-notes">备注</Label><Textarea id="score-notes" className="mt-2" rows={2} value={form.notes} onChange={(e) => setForm((v) => ({ ...v, notes: e.target.value }))} /></div>
