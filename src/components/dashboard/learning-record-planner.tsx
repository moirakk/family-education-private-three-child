      ...
      <CardContent className="space-y-4">
        ...
        {showForm && (
          <form onSubmit={saveRecord} className="rounded-2xl border border-border bg-card p-4">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {confidenceOptions.map((option) => (
                    <button
                      ...
                      className={cn(
                        "rounded-lg border px-2 py-1.5 text-xs font-medium",
                        form.confidence === option.value ? "border-foreground bg-foreground text-background" : "border-border bg-card text-muted-foreground"
                      )}
                    >
                      {option.label}
                    </button>
            ...
            {showAdvanced && (
              <div className="grid gap-3 rounded-2xl border border-border bg-muted/60 p-3 sm:grid-cols-2">
                <div className="space-y-1.5">
