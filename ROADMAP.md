...
- 必须先在生产 Supabase 执行 `docs/migrations/2026-07-17-*.sql`，再推送触发 Netlify 部署。
- 家长专属邀请链接和按家教/孩子/科目绑定的独立家教链接仍需完成，当前旧访问方式暂不作为本轮上线版本。
- 家教反馈页还需进一步压缩为日期、授课内容、表现、课后任务四个字段。
