...
create table if not exists public.learning_records (
  ...
  exam_type text not null default 'quiz' check (exam_type in ('quiz', 'monthly', 'midterm', 'final', 'other')),
  ...
  constraint learning_records_score_range check (score is null or score >= 0),
  constraint learning_records_max_score_positive check (max_score is null or max_score > 0),
...
create table if not exists public.education_goals (
  ...
  plan_type text not null default 'other' check (plan_type in ('exam', 'competition', 'school', 'other')),
  custom_type text,
